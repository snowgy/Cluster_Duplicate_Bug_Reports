...
if __name__ == '__main__':
    f = open('../Wednesday/formatted_result/t_result_2.json', 'r')
    tmp_t_result = json.load(f)
    f.close()
    f = open('../Wednesday/formatted_result/p_result_2.json', 'r')
    tmp_p_result = json.load(f)
    f.close()

    t_result = []
    for i in tmp_t_result:
        if len(i) != 0:
            t_result.append(i)

    p_result = []
    for i in tmp_p_result:
        if len(i) != 0:
            p_result.append(i)
    #
    # # f_r = open('', 'r')
