...
class Crawler:
    ...
    def fetch_data(self, stack_id):
        ...
        _field_container_component = edit_form.find(id="field_container_component").text.replace("(show other bugs)",
                                                                                                 "").strip()
...
def crawler(start, end):
    ...
    for id in range(start, end):
        ...
        result = _crawler.fetch_data(id)
        if result is not None:
            with open('jsons/stack_data-' + str(id) + '.json', 'w') as outfile:
                json.dump(result, outfile)


for i in range(1, 540000, 5000):
    t = threading.Thread(target=crawler, args=(i, i+5000))
    t.start()
